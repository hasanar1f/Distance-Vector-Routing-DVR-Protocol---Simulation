...
public class NetworkLayerServer {
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        */
        ...
        while (convergence){
            ...
            int ii = startingRouterId;
            while (true) {

                Router r = NetworkLayerServer.routers.get(ii-1);

                for(int nr_id : r.getNeighborRouterIDs()) {

                    if( NetworkLayerServer.routers.get(nr_id-1).getState() == true ) {

                        Router N = NetworkLayerServer.routers.get(nr_id-1);

                        for(int t : N.getNeighborRouterIDs() ) {

                             convergence = NetworkLayerServer.routers.get(t-1).sfupdateRoutingTable(r);
                ...
                ii = ii%routers.size() + 1;
                if(ii==startingRouterId) break;
    ...
    public static synchronized void simpleDVR(int startingRouterId) {
        ...
        while (convergence){
            ...
            while (true) {
                ...
                Router r = routerMap.get(ii);

                for(int nr_id : r.getNeighborRouterIDs()) {

                    if( NetworkLayerServer.routerMap.get(nr_id).getState() == true ) {

                        Router N = NetworkLayerServer.routerMap.get(nr_id);

                        for(int t : N.getNeighborRouterIDs() ) {

                            convergence = NetworkLayerServer.routerMap.get(t).updateRoutingTable(r);
    ...
    public static EndDevice getClientDeviceSetup() {
        ...
        System.out.println("Found a new Device with IP : " + ip + ":::: Gateway: " + gateway);
        return device;
    }
